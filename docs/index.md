
<img src="Images/mehedi_1.jpg" alt="Mehedi Hasan" width="450">

## Mehedi Hasan
**Doctoral Student & Graduate Research Associate**  
**Environmental Health Sciences**  
The University of Arizona, USA

## My Journey


<table>
  <tr>
    <td style="width:50%; vertical-align:top;">
      <p>
        Mehedi Hasan's journey in public health began in the vibrant yet challenging landscapes of Bangladesh, where he witnessed firsthand the pressing need for sustainable solutions. After earning his Bachelor's and Master's degrees in Public Health & Informatics at Jahangirnagar University, he spent five impactful years at the International Centre for Diarrhoeal Disease Research Bangladesh (icddr,b) working with marginalized communities to address critical WASH challenges.

        Now, as a second-year PhD student & Graduate Research Associate in UofA's Community, Environment, and Policy department, Mehedi is developing innovative health projects and mentoring future leaders, driven by a lifelong commitment to equity and global change.  

        **More in About...**
      </p>
    </td>
    <td style="width:50%; text-align:center;">
      <img src="Images/Interviewing_disability.jpg" alt="Interviewing a person with disability" width="450">
    </td>
  </tr>
</table>

*****
<div style="display: flex; align-items: center; gap: 20px; margin-top: 30px;">
  <img src="Images/academic.png" alt="Education" width="80">
  <div>
    <h3 style="margin: 0;">Education</h3>
    <p style="margin: 5px 0;">
      Explore my academic journey, from public health to environmental health sciences.
    </p>
    <a href="education/" style="color: #004F9F; font-weight: bold;">→ Go to Education Page</a>
  </div>
</div>
*****
<div style="display: flex; align-items: center; gap: 20px; margin-top: 30px;">
  <img src="Images/Experiences.png" alt="Experience Logo" width="80" style="vertical-align: middle;">
  <div>
    <h3 style="margin: 0;">Professional Experience</h3>
    <p style="margin: 5px 0;">
      Explore my roles at UofA , icddr,b, and other organizations where I’ve contributed to public health and environmental health research.
    </p>
    <a href="experience/" style="color: #004F9F; font-weight: bold; text-decoration: none;">→ View Experiences</a>
  </div>
</div>
*****
<div style="display: flex; align-items: center; gap: 20px; margin-top: 30px;">
  <img src="Images/Skills.png" alt="Skills Logo" width="80" style="vertical-align: middle;">
  <div>
    <h3 style="margin: 0;">Skills and Expertise</h3>
    <p style="margin: 5px 0;">
      Check out my skills and expertise
    </p>
    <a href="skills/" style="color: #004F9F; font-weight: bold; text-decoration: none;">→  Skills</a>
  </div>
</div>

<hr>

<div style="display: flex; flex-wrap: wrap; justify-content: space-between; align-items: center; padding: 20px 0; font-size: 0.9em; color: #555; border-top: 1px solid #ccc;">

  <!-- UA Logo -->
  <div style="flex: 1; min-width: 150px;">
    <img src="Images/UofA_logo.png" alt="University of Arizona Logo" width="100" style="vertical-align: middle;">
  </div>

  <!-- Contact Info -->
  <div style="flex: 2; min-width: 200px;">
    <p><strong>Contact:</strong><br>
    Mehedi Hasan<br>
    Email: <a href="mailto:mehedihasan@arizona.edu">mehedihasan@arizona.edu</a><br>
    Phone: +1 (520) 269 5013</p>
  </div>
  <!-- Social Media -->
  <div style="flex: 2; min-width: 200px; text-align: left;">
    <p><strong>Connect:</strong></p>
    <a href="http://www.linkedin.com/in/mehedi-hasan-68129813a" target="_blank" style="margin-right: 10px;">
      <img src="Images/Linkdin.png" alt="LinkedIn" width="24">
    </a>
    <a href="https://github.com/mehedi" target="_blank" style="margin-right: 10px;">
      <img src="Images/GitHub_Logo.png" alt="GitHub" width="24">
    </a>
    <a href="https://scholar.google.com/citations?user=LSdCFa4AAAAJ&hl=en" target="_blank">
      <img src="Images/scholar.png" alt="Google acholar" width="24">
    </a>
        <a href="https://www.facebook.com/profile.php?id=100008332921038&_rdc=1&_rdr#" target="_blank" style="margin-right: 10px;">
      <img src="Images/Facebook_logo.png" alt="Facebook" width="40">
    </a>
        <a href="https://orcid.org/0000-0002-6112-941X" target="_blank" style="margin-right: 40px;">
      <img src="Images/orcid-og-image.png" alt="Orchid ID" width="50">
    </a>
  </div> 
  <strong><I>"I'm always open to meaningful research collaborations—feel free to reach out anytime!"<I></strong>
</div>

